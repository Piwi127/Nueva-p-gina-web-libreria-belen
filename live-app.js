...
const SUBCATEGORY_PATTERNS = {
    ...
    utiles: [
        ...
        { key: 'lapices', label: 'LÃ¡pices', regex: /l[aÃ¡]piz/ },
        { key: 'colores', label: 'Colores', regex: /colores|cray[oÃ³]n/ },
        ...
        { key: 'reglas', label: 'Reglas y medidores', regex: /regla|escuadra|transportador|comp[aÃ¡]s/ },
    ...
    oficina: [
        ...
        { key: 'sellos', label: 'Sellos/TampÃ³n', regex: /tamp[oÃ³]n|sello/ },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (productGrid) {
        ...
        ensureProductsLoaded().then(() => {
            ...
            if (productGrid) {
                productGrid.innerHTML = '<p class="no-results" style="grid-column:1/-1; text-align:center; padding: 2rem;">No se pudo cargar el catÃ¡logo.</p>';
            }
        });
    }
...
function renderSearchDropdown(query, filtered) {
    ...
    if (q.length === 0) {
        ...
        if (history.length > 0) {
            searchResults.appendChild(createSectionTitle('BÃºsquedas recientes'));
        ...
        if (categoryList.length > 0) {
            searchResults.appendChild(createSectionTitle('CategorÃ­as populares'));
    ...
    if (relatedCategories.length > 0) {
        searchResults.appendChild(createSectionTitle('CategorÃ­as relacionadas'));
...
function categorizeByTitle(title) {
    ...
    if (/(cartulina|papel bond|papel lustre|papel crepe|papelÃ³grafo|papelografo|microporoso|papel fotocopia|papel autocopia|papel seda|papel kraft|papel carbÃ³n|papel carbon)/.test(t)) return 'papeleria';
    if (/(acuarela|tÃ©mpera|tempera|pintura|oleo|pincel|cray[oÃ³]n|plastilina|microporoso|foamy|eva|origami|escarcha|glitter|lentejuela)/.test(t)) return 'arte';
    if (/(borrador|tajador|sacapuntas|regla|transportador|comp[aÃ¡]s|pegamento|goma|silicona|tijera|cartuchera|estuche|colores|lapicero escolar|lapiz escolar)/.test(t)) return 'utiles';
    if (/(bol[Ã­i]grafo|lapicero|l[aÃ¡]piz|plum[oÃ³]n|marcador|resaltador|tinta|pilot|faber|bic|staedtler|paper mate)/.test(t)) return 'escritura';
    ...
    if (/(perforador|grapadora|engrap|tamp[oÃ³]n|sello|calculadora|laminadora|cinta adhesiva|dispensador)/.test(t)) return 'oficina';
    if (/(usb|memoria|mouse|teclado|aud[iÃ­]fono|cd|dvd|laptop|bater[iÃ­]a|cargador)/.test(t)) return 'tecnologia';
...
function buildSearchTokens({ title, description, longDescription, usage, category, subcategory, subcategoryLabel, brand }) {
    ...
    const usageAliases = {
        ...
        escribir: ['lapiz', 'lÃ¡piz', 'boligrafo', 'bolÃ­grafo', 'lapicero', 'pluma', 'tinta', 'marcador', 'plumon', 'plumÃ³n'],
        cortar: ['tijera', 'cutter', 'cortador'],
        medir: ['regla', 'escuadra', 'transportador', 'compas', 'compÃ¡s'],
        organizar: ['archivador', 'folder', 'carpeta', 'separador', 'organizador', 'clip', 'grapa', 'grapadora', 'chinche', 'alfiler'],
        colorear: ['colores', 'crayon', 'crayÃ³n', 'acuarela', 'tempera', 'tÃ©mpera', 'plumon', 'plumÃ³n', 'marcador'],
        ...
        colegio: ['cuaderno', 'block', 'lapiz', 'lÃ¡piz', 'borrador', 'tajador', 'sacapuntas', 'regla', 'pegamento', 'goma', 'colores'],
        arte: ['acuarela', 'tempera', 'tÃ©mpera', 'oleo', 'Ã³leo', 'pincel', 'crayon', 'crayÃ³n', 'plumon', 'plumÃ³n', 'papel dibujo', 'cartulina'],
        resaltar: ['resaltador', 'highlight', 'marcador fluorescente'],
        dibujar: ['lapiz', 'lÃ¡piz', 'plumon', 'plumÃ³n', 'marcador', 'crayon', 'crayÃ³n', 'block dibujo', 'papel dibujo'],
        ...
        embalar: ['cinta de embalaje', 'cinta transparente', 'cinta de empaque', 'caja', 'carton', 'cartÃ³n'],
        papel: ['cartulina', 'carton', 'cartÃ³n', 'papel lustre', 'papel seda', 'papel crepe', 'papel bond', 'papel fotocopia', 'resma'],
        rotular: ['marcador', 'plumon', 'plumÃ³n', 'rotulador', 'etiqueta', 'label'],
...
function openProductModal(productId) {
    ...
    if (detailAddBtn) {
        ...
        detailAddBtn.title = PRICES_PENDING ? 'Precios prÃ³ximamente' : '';
    }
    ...
    if (product.features && product.features.length > 0) {
        ...
        featureList.innerHTML = '<li>Sin informaciÃ³n adicional.</li>';
    }
    ...
    if (product.usage && product.usage.length > 0) {
        ...
        usageList.innerHTML = '<li>Sin informaciÃ³n de uso.</li>';
    }
...
function addToCartFromDetail() {
    if (PRICES_PENDING) {
        showToast('Precios prÃ³ximamente', 'error');
        return;
    }
...
function getCustomerData() {
    ...
    if (consentInput && !consentInput.checked) {
        alert('Debes aceptar el envÃ­o de datos por WhatsApp para continuar.');
        return null;
    }
...
function checkout() {
    ...
    if (PRICES_PENDING) {
        alert('Los precios estÃ¡n por definirse. Pronto podrÃ¡s comprar.');
        return;
    }
    ...
    let message = "Hola *LIBRERÃA BELEN*, deseo realizar el siguiente pedido:\n\n";
    ...
    message += "\n\nÂ¿CuÃ¡les son los mÃ©todos de pago disponibles y el tiempo de entrega?";
...
async function generateInvoice() {
    if (cart.length === 0) {
        alert('El carrito estÃ¡ vacÃ­o. Agrega productos para generar una boleta.');
        return;
    }

    if (PRICES_PENDING) {
        alert('Los precios estÃ¡n por definirse. La boleta estarÃ¡ disponible prÃ³ximamente.');
        return;
    }
    ...
    doc.text("LIBRERÃA BELEN", 105, 20, null, null, "center");
    ...
    doc.setTextColor(100);
    doc.text("Â¡Gracias por su compra!", 105, y + 20, null, null, "center");
...
function initSupportRequestForm() {
    ...
    form.addEventListener('submit', (event) => {
        ...
        if (!serviceType || !deviceType || !brandModel || !issueDetail || !urgency || !customerName || !customerPhone) {
            alert('Por favor, completa todos los campos obligatorios del formulario tÃ©cnico.');
            return;
        }

        const normalizedPhone = customerPhone.replace(/\D/g, '');
        if (normalizedPhone.length < 9) {
            alert('Ingresa un nÃºmero de contacto vÃ¡lido (mÃ­nimo 9 dÃ­gitos).');
            return;
        }
        ...
        let message = 'Hola LIBRERÃA BELÃN, deseo solicitar SOPORTE TÃCNICO para PC:\n\n';
        ...
        message += '\nQuedo atento(a) a su evaluaciÃ³n y propuesta tÃ©cnica.';
...
function toggleFavorite(productId) {
    ...
    if (index > -1) {
        ...
        showToast('Â¡Producto aÃ±adido a favoritos!', 'success');
    }
...
function addToCart(productId) {
    ...
    showToast(`${product.title} aÃ±adido al carrito`, 'success');
}
...
function getProductCardHtml(product, index) {
    ...
    const safeImage = escapeAttr(sanitizeUrl(product.image));
    const cartDisabled = PRICES_PENDING ? 'disabled aria-disabled="true" title="Precios prÃ³ximamente"' : '';
...
function updateCatalogInsights() {
    ...
    const chips = [];
    if (currentCategory !== 'all') chips.push({ label: `CategorÃ­a: ${CATEGORY_LABELS[currentCategory] || currentCategory}`, clear: 'category' });
    if (currentSubcategory !== 'all') chips.push({ label: `SubcategorÃ­a: ${getSubcategoryLabel(currentCategory, currentSubcategory)}`, clear: 'subcategory' });
    if (currentBrand !== 'all') chips.push({ label: `Marca: ${currentBrand}`, clear: 'brand' });
    if (currentSearchQuery.trim()) chips.push({ label: `BÃºsqueda: ${currentSearchQuery.trim()}`, clear: 'search' });
